...
func sMail(sub, content, mailTo, serverAddr, user, password string, tls bool) error {
	host, port, _ := net.SplitHostPort(serverAddr)
	nPort, _ := strconv.Atoi(port)
	if tls {
		err := common.Mail(sub, content, mailTo, host, nPort, user, password)
		if err != nil {
			fmt.Println(err)
			return err
		}
